			...
			case WAVE_ANALYSIS:
					if(bWaveOver)
					{
						...
						if(WAVE_ANLS->GetWaveFreq(true,dwTemp) != WAVE_FREQ_PASS
							|| CrestVerify(true,dwTemp) != WAVE_CREST_NUMBER_PASS)
						{
							plDUT->Caption = "(!)WaveOut Waveform Error";
							plDUT->Font->Color = clRed;
							bLeftError = true;
						}
			...
			case WAVE_ANALYSIS_2:
					if(bWaveOver)
					{
						...
						if(WAVE_ANLS->GetWaveFreq(true,dwTemp) != WAVE_FREQ_PASS
							 || CrestVerify(true,dwTemp) != WAVE_CREST_NUMBER_PASS)
						{
							if(	plDUT->Caption.Pos("Error"))
								plDUT->Caption = "(!)WaveOut&WaveIn Waveform Error";
							else plDUT->Caption = "(!)WaveIn Waveform Error";
							plDUT->Font->Color = clRed;
							bRightError = true;
						}
